...
type JsonOpts = { timeoutMs?: number };
...
export const aiLayers = {
  ...
  async generateCreative(prompt: string, opts: { timeoutMs?: number } = {}) {
    return aiProvider.generateText(prompt, { timeoutMs: opts.timeoutMs ?? 20000 });
  },
  ...
  async generateStructuredJson(prompt: string, opts: JsonOpts = {}) {
    ...
    return aiProvider.generateJsonRaw(prompt, { timeoutMs: opts.timeoutMs ?? 12000 });
  },

  // Short explanations / reasons
  async explain(prompt: string, opts: { timeoutMs?: number } = {}) {
    return aiProvider.generateText(prompt, { timeoutMs: opts.timeoutMs ?? 8000 });
  },
};
